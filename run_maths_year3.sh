#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Maths Year 3 — 1-worker scraper
#  Usage:
#    Step 1 (once):   bash run_maths_year3.sh setup
#    Step 2 (always): bash run_maths_year3.sh worker0
#
...
SUBJECT="maths"
YEAR="year-3"
OUT="output/maths/year-3"

case "$1" in

  setup)
    echo "Fetching Year 3 Maths skill list..."
...
print('Now run: bash run_maths_year3.sh worker0')
"
    ;;
  ...
  *)
    echo "Usage: bash run_maths_year3.sh [setup|worker0]"
    ;;
