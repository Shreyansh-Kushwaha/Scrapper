#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Science Year 8 — 1-worker scraper
#  Usage:
#    Step 1 (once):   bash run_science_year8.sh setup
#    Step 2 (always): bash run_science_year8.sh worker0
#
...
SUBJECT="science"
YEAR="year-8"
OUT="output/science/year-8"

case "$1" in

  setup)
    echo "Fetching Year 8 Science skill list..."
...
print('Now run: bash run_science_year8.sh worker0')
"
    ;;
  ...
  *)
    echo "Usage: bash run_science_year8.sh [setup|worker0]"
    ;;
