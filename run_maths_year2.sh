#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Maths Year 2 — 1-worker scraper
#  Usage:
#    Step 1 (once):   bash run_maths_year2.sh setup
#    Step 2 (always): bash run_maths_year2.sh worker0
#
...
SUBJECT="maths"
YEAR="year-2"
OUT="output/maths/year-2"

case "$1" in

  setup)
    echo "Fetching Year 2 Maths skill list..."
...
print('Now run: bash run_maths_year2.sh worker0')
"
    ;;
  ...
  *)
    echo "Usage: bash run_maths_year2.sh [setup|worker0]"
    ;;
