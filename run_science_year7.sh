#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Science Year 7 — 1-worker scraper
#  Usage:
#    Step 1 (once):   bash run_science_year7.sh setup
#    Step 2 (always): bash run_science_year7.sh worker0
#
...
SUBJECT="science"
YEAR="year-7"
OUT="output/science/year-7"

case "$1" in

  setup)
    echo "Fetching Year 7 Science skill list..."
...
print('Now run: bash run_science_year7.sh worker0')
"
    ;;
  ...
  *)
    echo "Usage: bash run_science_year7.sh [setup|worker0]"
    ;;
