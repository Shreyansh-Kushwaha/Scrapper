#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Year 8 English — 2-worker scraper
#  Usage:
#    Step 1 (once):   bash run_year8.sh setup
#    Step 2 (always): bash run_year8.sh worker0    ← Terminal 1
#    Step 3 (always): bash run_year8.sh worker1    ← Terminal 2
#
...
SUBJECT="english"
YEAR="year-8"
OUT="output/english/year-8"

case "$1" in

  setup)
    echo "Fetching Year 8 skill list and splitting into 2 batches..."
...
print('Now run:  bash run_year8.sh worker0   (Terminal 1)')
print('     and: bash run_year8.sh worker1   (Terminal 2, wait ~10s)')
"
    ;;
  ...
  *)
    echo "Usage: bash run_year8.sh [setup|worker0|worker1]"
    echo ""
