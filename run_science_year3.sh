#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Science Year 3 — 1-worker scraper
#  Usage:
#    Step 1 (once):   bash run_science_year3.sh setup
#    Step 2 (always): bash run_science_year3.sh worker0
#
...
SUBJECT="science"
YEAR="year-3"
OUT="output/science/year-3"

case "$1" in

  setup)
    echo "Fetching Year 3 Science skill list..."
...
print('Now run: bash run_science_year3.sh worker0')
"
    ;;
  ...
  *)
    echo "Usage: bash run_science_year3.sh [setup|worker0]"
    ;;
