#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Year 11 English — 2-worker scraper
#  Usage:
#    Step 1 (once):   bash run_year11.sh setup
#    Step 2 (always): bash run_year11.sh worker0    ← Terminal 1
#    Step 3 (always): bash run_year11.sh worker1    ← Terminal 2
#
...
SUBJECT="english"
YEAR="year-11"
OUT="output/english/year-11"

case "$1" in

  setup)
    echo "Fetching Year 11 skill list and splitting into 2 batches..."
...
print('Now run:  bash run_year11.sh worker0   (Terminal 1)')
print('     and: bash run_year11.sh worker1   (Terminal 2, wait ~10s)')
"
    ;;
  ...
  *)
    echo "Usage: bash run_year11.sh [setup|worker0|worker1]"
    echo ""
