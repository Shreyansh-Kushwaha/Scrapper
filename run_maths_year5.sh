#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Maths Year 5 — 3-worker scraper
#  Usage:
#    Step 1 (once):   bash run_maths_year5.sh setup
#    Step 2 (always): bash run_maths_year5.sh worker0    ← Terminal 1
#    Step 3 (always): bash run_maths_year5.sh worker1    ← Terminal 2
#    Step 4 (always): bash run_maths_year5.sh worker2    ← Terminal 3
#
...
SUBJECT="maths"
YEAR="year-5"
OUT="output/maths/year-5"

case "$1" in

  setup)
    echo "Fetching Year 5 Maths skill list and splitting into 3 batches..."
...
"
  ...
  *)
    echo "Usage: bash run_maths_year5.sh [setup|worker0|worker1|worker2]"
    ;;
