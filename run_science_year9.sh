#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Science Year 9 — 1-worker scraper
#  Usage:
#    Step 1 (once):   bash run_science_year9.sh setup
#    Step 2 (always): bash run_science_year9.sh worker0
#
...
SUBJECT="science"
YEAR="year-9"
OUT="output/science/year-9"

case "$1" in

  setup)
    echo "Fetching Year 9 Science skill list..."
...
print('Now run: bash run_science_year9.sh worker0')
"
    ;;
  ...
  *)
    echo "Usage: bash run_science_year9.sh [setup|worker0]"
    ;;
