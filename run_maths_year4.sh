#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Maths Year 4 — 3-worker scraper
#  Usage:
#    Step 1 (once):   bash run_maths_year4.sh setup
#    Step 2 (always): bash run_maths_year4.sh worker0    ← Terminal 1
#    Step 3 (always): bash run_maths_year4.sh worker1    ← Terminal 2
#    Step 4 (always): bash run_maths_year4.sh worker2    ← Terminal 3
#
...
SUBJECT="maths"
YEAR="year-4"
OUT="output/maths/year-4"

case "$1" in

  setup)
    echo "Fetching Year 4 Maths skill list and splitting into 3 batches..."
...
"
  ...
  *)
    echo "Usage: bash run_maths_year4.sh [setup|worker0|worker1|worker2]"
    ;;
