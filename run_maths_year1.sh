#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Maths Year 1 — 1-worker scraper
#  Usage:
#    Step 1 (once):   bash run_maths_year1.sh setup
#    Step 2 (always): bash run_maths_year1.sh worker0
#
...
SUBJECT="maths"
YEAR="year-1"
OUT="output/maths/year-1"

case "$1" in

  setup)
    echo "Fetching Year 1 Maths skill list..."
...
print('Now run: bash run_maths_year1.sh worker0')
"
    ;;
  ...
  *)
    echo "Usage: bash run_maths_year1.sh [setup|worker0]"
    ;;
