#!/bin/bash
# ─────────────────────────────────────────────────────────────────
#  Aus Science Year 1 — 1-worker scraper
#  Usage:
#    Step 1 (once):   bash run_science_year1.sh setup
#    Step 2 (always): bash run_science_year1.sh worker0
#
...
SUBJECT="science"
YEAR="year-1"
OUT="output/science/year-1"

case "$1" in

  setup)
    echo "Fetching Year 1 Science skill list..."
...
print('Now run: bash run_science_year1.sh worker0')
"
    ;;
  ...
  *)
    echo "Usage: bash run_science_year1.sh [setup|worker0]"
    ;;
